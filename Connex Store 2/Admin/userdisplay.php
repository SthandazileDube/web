...
if(isset($_GET['delete'])){
    ...
    $delete_query = mysqli_query($conn, "DELETE FROM `register` where id = $delete_id");
    if($delete_query){
...
<div>
    ...
    <thead class="bg-secondary text-dark">
        ...
        <th class="text-center" style="color: black;">id</th>
            ...
            <th class="text-center" style="color: black;">Lastname</th>
            <th class="text-center" style="color: black;">Email</th>
            <th class="text-center" style="color: black;">Password</th>
            <th class="text-center" style="color: black;">Type</th>
        ...
        if($result = mysqli_query($conn, $sql)){
            if(mysqli_num_rows($result) > 0){
                while($row = mysqli_fetch_array($result)){
                  ...
                  <td style="font-size: 14px;"><?= $row["lastname"]?></td>
                  <td style="font-size: 14px;"><?= $row["email"]?></td>
                  <td style="font-size: 14px;"><?= $row["password"]?></td>
                  <td style="font-size: 14px;"><?= $row["user_type"]?></td>
                  
                   <td> <a href="userdisplay.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are you sure you want to delete this?');"><i class="fas fa-trash"></i>delete</a></td>
                  </tr>
                  <?php
                }
            }
